...
export const calculateFraud = (
  ...
  // ticket.timeAdded is ISO 8601, converted to String (using in the ticketTimeAdded variable)
  const hrsString = ticketTimeAdded.split(" ")[4].split(":")[0];
  const hrs = parseInt(hrsString, 10);
  const avgPriceAllTickets =
    allTickets.reduce((a, b) => a + b.price, 0) / allTickets.length;
  const percPrice = (
    ((avgPriceAllTickets - ticket.price) / ticket.price) *
    100
  ).toFixed(0);
